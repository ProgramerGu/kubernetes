...
// This annotation is added to a PVC that has been triggered by scheduler to
// be dynamically provisioned. Its value is the info of the bound pod.
const annoBoundPod = "volume.kubernetes.io/bound-pod"
...
//getClaimByName XX
func (ctrl *PersistentVolumeController) getClaimByName(ns, name string) (*v1.PersistentVolumeClaim, error) {
	return ctrl.kubeClient.CoreV1().PersistentVolumeClaims(ns).Get(name, metav1.GetOptions{})
}

//updateClaimAnnoBoundPod XX
func (ctrl *PersistentVolumeController) updateClaimAnnoBoundPod(claim *v1.PersistentVolumeClaim, podString string) {
	klog.V(4).Infof("guyutest updateClaimAnnoBoundPod pvc %s/%s pod %s", claim.Namespace, claim.Name, podString)
	cloneClaim := claim.DeepCopy()
	if _, ok := cloneClaim.Annotations[annoBoundPod]; ok {
		delete(cloneClaim.Annotations, annoBoundPod)
	}
	metav1.SetMetaDataAnnotation(&cloneClaim.ObjectMeta, annoBoundPod, podString)

	_, err := ctrl.kubeClient.CoreV1().PersistentVolumeClaims(cloneClaim.Namespace).Update(cloneClaim)
	if err != nil {
		klog.V(4).Infof("Failed to update annotation 'annBoundPod' for PersistentVolumeClaim %q: %v", claimToClaimKey(cloneClaim), err)
		return
	}
}
