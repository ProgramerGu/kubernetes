...
func (b *volumeBinder) AssumePodVolumes(assumedPod *v1.Pod, nodeName string) (allFullyBound bool, err error) {
	...
	if allBound := b.arePodVolumesBound(assumedPod); allBound {
		...
		//Update podinfo in pvc annotation for reservation
		b.checkAndUpdatePVCAnnotation(assumedPod)
		return true, nil
	}
	...
	for _, claim := range claimsToProvision {
		...
		//Add pod info to pvc annotations for reservation
		podByte, err1 := json.Marshal(assumedPod)
		if err1 == nil {
			metav1.SetMetaDataAnnotation(&claimClone.ObjectMeta, annoBoundPod, string(podByte))
		} else {
			klog.V(4).Infof("Add Annotations to PVC %s fail : %s", claim.GetName(), err1)
		}
...
func (b *volumeBinder) checkAndUpdatePVCAnnotation(assumedPod *v1.Pod) {
	klog.V(4).Infof("guyutest checkAndUpdatePVCAnnotation %s/%s", assumedPod.Namespace, assumedPod.Name)
	for _, volume := range assumedPod.Spec.Volumes {
		if volume.PersistentVolumeClaim != nil {
			if volume.PersistentVolumeClaim.ClaimName != "" {
				pvcName := volume.PersistentVolumeClaim.ClaimName
				claim, err := b.ctrl.getClaimByName(assumedPod.Namespace, pvcName)
				if err != nil {
					klog.V(4).Infof("get %s/%s pvc %s fail %s", assumedPod.Namespace, assumedPod.Name, pvcName, err)
					continue
				}
				//if claim.Annotations != nil {
				//if strings.Contains(claim.Annotations[util.AnnoBoundPod], "importer") && !strings.Contains(claim.Annotations[util.AnnoBoundPod], assumedPod.Name) {
				podByte, err := json.Marshal(assumedPod)
				if err == nil {
					b.ctrl.updateClaimAnnoBoundPod(claim, string(podByte))
				} else {
					klog.V(4).Infof("Marshal pod %s/%s error %s", assumedPod.Namespace, assumedPod.Name, err)
				}
				//}
				//}
			}
