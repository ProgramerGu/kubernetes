...
//InitReservation add reserve pod by pvc list
func InitReservation(pVCLister corelisters.PersistentVolumeClaimLister, schedulerCache schedulerinternalcache.Cache) {
	err := util.ReserverInit(pVCLister, schedulerCache)
	if err != nil {
		klog.V(4).Infof("ReserverInit fail %s", err)
	}

	go func() {
		time.Sleep(10 * time.Second)
		for {
			err = util.Reserver(pVCLister, schedulerCache)
			if err != nil {
				klog.V(4).Infof("Reserver fail %s", err)
			}
			time.Sleep(10 * time.Second)
		}
	}()
